...
import blue.lhf.lambda_calculus.parser.tokeniser.Tokenizer.Lambda;
import blue.lhf.lambda_calculus.parser.tokeniser.Tokenizer.LeftParen;
import blue.lhf.lambda_calculus.parser.tokeniser.Tokenizer.Space;
import blue.lhf.lambda_calculus.parser.tokeniser.Tokenizer.VariableToken;
	...
	private Expression parseExpression(final ListIterator<Token> tokens, final Map<String, Variable> boundVariables) {
		...
		do {
			switch (tokens.next()) {
				...
				case Lambda ignored -> {
					final List<VariableToken> variableTokens = new ArrayList<>();
					while (tokens.hasNext() && (tokens.next() instanceof VariableToken v)) {
					...
					for (VariableToken variableToken : variableTokens) {
		...
		} while (skipToNextSubExpression(tokens));
